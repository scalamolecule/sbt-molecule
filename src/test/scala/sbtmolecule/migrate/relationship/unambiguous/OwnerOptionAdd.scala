...
trait OwnerOptionAdd_1 extends DomainStructure {
  trait Order {
    val number   = oneInt
...
trait OwnerOptionAdd_2 extends DomainStructure {
  trait Order {
    val number   = oneInt
...
object OwnerOptionAdd extends BaseTest {
  ...
  override def tests: Tests = Tests {
    ...
    "SQL" - {
      ...
      // Should drop old FK constraint and add new one with CASCADE
      assert(sqlText.contains("ALTER TABLE Order_ DROP CONSTRAINT _customer"))
      assert(sqlText.contains("ALTER TABLE Order_ ADD CONSTRAINT _customer FOREIGN KEY (customer) REFERENCES Customer (id) ON DELETE CASCADE"))
      ...
      val fkAddPos  = sqlText.indexOf("ALTER TABLE Order_ ADD CONSTRAINT _customer FOREIGN KEY (customer) REFERENCES Customer (id) ON DELETE CASCADE")
    ...
    "All SQL" - {
      sql(before, after) ==>
        """ALTER TABLE Order_ DROP CONSTRAINT _customer;
          |ALTER TABLE Order_ ADD CONSTRAINT _customer FOREIGN KEY (customer) REFERENCES Customer (id) ON DELETE CASCADE;""".stripMargin
    }
  }
}
