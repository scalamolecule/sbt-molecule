...
case class Schema_Sql2(schema: MetaSchema) extends RegexMatching with BaseHelpers {

  private val nss: Seq[MetaNs] = schema.parts.flatMap(_.nss)

  private def createTable(metaNs: MetaNs, field: Field): Seq[String] = {
    val ns     = metaNs.ns
    val attrs  = metaNs.attrs.filterNot(a => a.card == CardSet && a.refNs.nonEmpty)
    val max    = attrs.map(a => a.attr.length).max.max(2)
    val fields = attrs
      .map(a => a.attr + padS(max, a.attr) + " " + field.tpe(a))
      .mkString(s",\n|      |  ")
    val table  =
  ...
  def tables(dialect: Field): String = nss.flatMap(createTable(_, dialect)).mkString("\n|      |")
