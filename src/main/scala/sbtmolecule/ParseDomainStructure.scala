...
object ParseDomainStructure {
  def apply(filePath: String): MetaDomain = {
    val bytes   = Files.readAllBytes(Paths.get(filePath))
    val content = new String(bytes, "UTF-8")
    new ParseDomainStructure(filePath, content).metaDomain
  }
}

class ParseDomainStructure(filePath: String, content: String) extends BaseHelpers {
