...
case class ExprSeqMan(
  ...
  val enu = if (hasEnum)
    s"""
       ...
       |  def add   (v   : T, vs: T*  ) = new $entA[$tpe2](addSeq(dataModel, Add    , (Seq(v) ++ vs).map(_.toString.asInstanceOf[T]))) with CardSeq
       |  def add   (vs  : Iterable[T]) = new $entA[$tpe2](addSeq(dataModel, Add    , (Seq()  ++ vs).map(_.toString.asInstanceOf[T]))) with CardSeq
       |  def remove(v   : T, vs: T*  ) = new $entA[$tpe2](addSeq(dataModel, Remove , (Seq(v) ++ vs).map(_.toString.asInstanceOf[T]))) with CardSeq
       |  def remove(vs  : Iterable[T]) = new $entA[$tpe2](addSeq(dataModel, Remove , (Seq()  ++ vs).map(_.toString.asInstanceOf[T]))) with CardSeq
       |}
       |""".stripMargin
  else ""
  ...
  def get: String =
    s"""
       ...
       |  def add   (v   : T, vs: T*  ) = new $entA[$tpe2](addSeq(dataModel, Add    , Seq(v) ++ vs)) with CardSeq
       |  def add   (vs  : Iterable[T]) = new $entA[$tpe2](addSeq(dataModel, Add    , Seq()  ++ vs)) with CardSeq
       |  def remove(v   : T, vs: T*  ) = new $entA[$tpe2](addSeq(dataModel, Remove , Seq(v) ++ vs)) with CardSeq
       |  def remove(vs  : Iterable[T]) = new $entA[$tpe2](addSeq(dataModel, Remove , Seq()  ++ vs)) with CardSeq
       |
