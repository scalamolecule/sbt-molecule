...
case class Entity_Attrs(
  ...
  def get: String = {
    ...
    attributes.collect {
      case MetaAttribute(attr, value, tpe, _, optRef, _, _, _, _, optAlias, _, valueAttrs, validations, _, _, _, _) =>
        val cleanAttr = optAlias.getOrElse(attr)
        val valids    = if (validations.nonEmpty) {
          val valueAttrMetas = attributes.collect {
            case MetaAttribute(attr1, value1, tpe1, _, _, _, _, _, _, _, _, _, _, _, _, _, _)
