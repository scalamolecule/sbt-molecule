...
case class Entity_Builder(
  ...
  attributes.foreach {
    case MetaAttribute(attr, value, baseType0, _, optRef, _, _, optEnum, _, optAlias, _, _, _, _, _, _, _) =>
  ...
  refs.collect {
    case MetaAttribute(attr, value, _, _, Some(ref0), Some(reverseRef), Some(relationship), _, options, optAlias, _, _, _, _, _, _, _) =>
