...
//
//  arity: Int
//) extends FormatGraphql(typeNames, enumNames, entity, fields, arity) {
//
//  private val man = List.newBuilder[String]
//  private val opt = List.newBuilder[String]
//  private val ref = List.newBuilder[String]
//
//  private val last         = arity == maxArity
//  private var hasOne       = false
//  private var hasSeq       = false
//  private var hasByteArray = false
//  private var seqCount     = 0
//
//  private var hasRefOne  = false // refs.exists(_.card == CardOne)
//  private var hasRefMany = false // refs.exists(_.card == CardSet)
////  private var refs       = List.empty[(String, String, String)]
//
//  private val ptMax = {
//    attrs.collect {
...
//              // rawType is a ref type
//              hasRefOne = true
//              refs = refs :+ (attr, card, rawType)
//            }
//            baseTpe.length
//
//          case (_, baseTpe) =>
...
//
//  attrs.collect {
//    case Attr(description, attr, args, card, rawType, tpe, mandatory, ref) =>
//      val padA = padAttr(attr)
//      val pad1 = padType(tpe)
//
//      val (tM, tO) = card match {
//        case "One"              => (tpe + ptOne(tpe), s"Option[$tpe]" + ptOne(tpe))
//        case _ if tpe == "Byte" => (s"Array[$tpe]" + ptByteArray, s"Option[Array[$tpe]]" + ptByteArray)
//        case _                  => (s"Seq[$tpe]" + ptSeq(tpe), s"Option[Seq[$tpe]]" + ptSeq(tpe))
//      }
//
...
//
//  refs.collect {
//    case Attr(description, attr, args, card, rawType, baseType, mandatory, Some(ref0)) =>
...
//        Some(s"""object _$backRef$pad extends $backRef${_0}$pad[${`A..V, `}t](dataModel.add(_dm.BackRef("$backRef", "$ent")))""")
//      }
//    }.mkString("\n\n  ", "\n  ", "")
//  }
//
