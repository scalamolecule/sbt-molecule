...
case class NsArityLevel(
  ...
  level: Int,
  genericPkg: String
) extends Helpers(model, namespace, in, out, level, genericPkg) {
  ...
  def getRef(r: Ref) = {
    ...
    val nestedDef    = if (clazz2 == "ManyRef" && out < maxOut) {
      ...
      Seq(s"Nested_${in}_${nn(out)}[$ns_ref_, " + `p0 with o1[p1]` + s", $nsTypes" + `, I1, A` + "]")
    } else {
      Nil
    }
  ...
  val man = mutable.MutableList.empty[String]
  val opt = mutable.MutableList.empty[String]
  val tac = mutable.MutableList.empty[String]

  val manK       = mutable.MutableList.empty[String]
  val tacK       = mutable.MutableList.empty[String]
  ...
  attrs.foreach {
    case a: Val =>
      ...
      if (out < maxOut) {
        ...
        if (isGeneric) {
          if (isSchema && Seq("doc", "index", "unique", "fulltext", "isComponent", "noHistory").contains(a.attr))
  ...
  val (datomNss, datomNs) = if (isDatom) {
  ...
  val attrsNext: Seq[String] = if (out < maxOut) {
    man ++ Seq("") ++
      (if (manK.nonEmpty) manK ++ Seq("") else Nil) ++
      opt ++ Seq("")
  } else Nil
  val attrKStay: Seq[String] = if (tacK.nonEmpty) Seq("") ++ tacK else Nil
  ...
  val body = (attrsNext ++ tac ++ attrKStay ++ refs ++ backRefs ++ selfJoin).mkString("\n  ").trim
  ...
  val tpeOverride = if(isGeneric)" " else " override"

  val typeNext = if (out == maxOut) "" else
    s"$tpeOverride type Next[Attr[_, _], Prop, Tpe] = Attr[$next0, $next1] with $next0\n "

  val typeStay =
    s"$tpeOverride type Stay[Attr[_, _], Prop, Tpe] = Attr[$this0, $this1] with $this0"
